...
interface DayEditPanelProps {
  ...
  drafts: DraftEntry[]
  saving: boolean
  onDraftChange: (idx: number, field: 'projectId' | 'duration', value: string | number) => void
...
export function DayEditPanel({
  ...
  const canAddDraft = dayRemaining > 0 && projects.filter((p) => !drafts.map((d) => d.projectId).includes(p.id)).length > 0
  ...
  return (
    <AppSidePanel
      ...
      title={
        selectedDayDate ? (
          <span className="flex flex-col gap-0.5 sm:flex-row sm:flex-wrap sm:items-baseline sm:gap-x-1.5">
            <span className="capitalize">{format(selectedDayDate, 'EEEE', { locale: fr })}</span>
      ...
      footer={
        <Button variant="outline" size="sm" className="w-full" disabled={saving} onClick={onSave}>
    ...
    >
      ...
      <div className="app-summary-box mb-4 overflow-hidden">
        <div className="app-summary-row mb-2">
          <span className="app-summary-label shrink-0">Avancement semaine</span>
          <span className="app-summary-value text-xs min-w-0 truncate">
            {formatHoursLabel(weekTotalHours)} / {target}h
            {target - weekTotalHours > 0 && (
              <span className="font-normal text-muted-foreground"> · {formatHoursLabel(target - weekTotalHours)} dispo</span>
            )}
          </span>
        </div>
        <Progress
          value={target > 0 ? (weekTotalHours / target) * 100 : 0}
          className={cn('h-1.5 bg-muted', weekTotalHours >= target ? '[&>div]:bg-green-500' : '[&>div]:bg-blue-500')}
        />
      </div>

      <div className="space-y-3">
        {drafts.length === 0 && (
          <p className="py-4 text-center text-xs text-muted-foreground">Aucune activité saisie pour ce jour.</p>
        )}
        {drafts.map((draft, idx) => (
          <div key={idx} className="flex items-center gap-2 rounded-lg border border-border bg-background p-2 overflow-hidden">
        ...
        {drafts.length > 0 && (
          <div className="app-summary-row pt-2 px-0.5">
            ...
            <span className={cn('text-sm font-semibold tabular-nums', dayHoursTextClass(dayTotal))}>
