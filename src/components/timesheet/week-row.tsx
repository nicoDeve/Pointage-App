...
export function WeekRow({ week, entries, projects, isCurrent, isPast, onSelect, ref }: WeekRowProps & { ref?: React.Ref<HTMLTableRowElement> }) {
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        ...
        >
              ...
              <HoverCard openDelay={150} closeDelay={100}>
                ...
                <HoverCardContent className="w-64 p-0 shadow-xl border border-border/50 overflow-hidden" side="right" align="start">
                  <div className="px-4 py-3 bg-linear-to-r from-muted/80 to-muted/40 border-b border-border/50">
                    <p className="font-semibold text-foreground">Semaine {week.week}</p>
                    ...
                    {Array.from(projectStats.entries()).map(([pid, h]) => {
                      ...
                      return (
                        <div key={pid} className="flex items-center justify-between">
                          ...
                          <span className="font-semibold text-foreground">{formatHoursLabel(h)}</span>
                        </div>
                      )
                    })}
                  </div>
          ...
          {/* Hours + status */}
          <TableCell className="w-22 px-3 py-2.5 text-right">
            <div className="flex flex-col items-end gap-1.5">
              <span className="font-semibold text-foreground tabular-nums">
                {formatHoursLabel(hours)}<span className="text-xs text-muted-foreground">/{target}h</span>
              </span>
